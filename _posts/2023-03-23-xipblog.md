---
layout: post
title: "[CVE-2023-27847] Multiple improper neutralizations of an SQL parameters in xipblog module for PrestaShop"
categories: modules
author:
- 202-ecommerce.com
- sorcery.ie
...
In the blog module (xipblog), an anonymous user can perform SQL injection. Releases are not properly managed, so latest 2.0.1 could not fixed vulnerabilities.
...
In xipblog module for PrestaShop up to 2.0.1, multiple sensitive SQL calls on `xipcategoryclass` and `xippostsclass` can be executed with a trivial http call and exploited to forge a blind SQL injection throught for instance the POST or GET submitted "rewrite" variable.

**WARNING**: this module is actively exploited by malicious network to hijacked shop including webskimmers deploys (auto-substitution of payment's method to massively stole credit carts).
...
Created by [Emme from sorcery.ie](https://patch-diff.githubusercontent.com/raw/xpert-idea/xipblog/pull/30.patch)
