...
Prerequisite : 
- The field `phone_mobile` within table gls_cart_carrier suffer of a type varchar(255) which is large enough to allow dangerous XSS payloads.

Steps : 
1/2 : The method `NkmGlsCheckoutModuleFrontController::displayAjaxSavePhoneMobile` do not properly clean the parameter `gls_customer_mobile`. pSQL is useless against XSS which exploit HTML tag attributes (Category 2 according to OWASP - pSQL only neutralized Category 1 thanks to its strip_tags).
2/2 : The output in the backoffice is not escaped in the related smarty template which use it.
