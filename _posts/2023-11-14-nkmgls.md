...
Steps : 
1/2 : The method `NkmGlsCheckoutModuleFrontController::displayAjaxSavePhoneMobile` does not properly clean the parameter `gls_customer_mobile`. pSQL is useless against XSS which exploits HTML tag attributes (Category 2 according to OWASP - pSQL only neutralized Category 1 thanks to its strip_tags).
2/2 : The output in the backoffice is not escaped in the related smarty template that uses it.
